...
function FirstSelectionSort(array) {
    for (let i = 0; i < array.length - 1; i++) {
        //We initialize a variable minIndex to i. This variable will keep track
        // of the index of the minimum element found during each pass of the array.
        let minIndex = i;
        //We start another loop nested inside the outer loop. This inner loop iterates
        //through the array, starting from the element just after the current i
...
function selectionSort(array, compareFunction) {
    //We start a loop that iterates through the array, from the first element
    //(index 0) to the second-to-last element. The reason we stop at the
    //second-to-last element is that the last element will already be in its
    //correct position when all others are sorted.
    for (let i = 0; i < array.length - 1; i++) {
        //We initialize a variable minIndex to i. This variable will keep track
        // of the index of the minimum element found during each pass of the array.
        let minIndex = i;
        //We start another loop nested inside the outer loop. This inner loop
        //iterates through the array, starting from the element just after the current i
        for (let j = i + 1; j < array.length; j++) {
...
function findMinLocation(array, startIndex, compareFunction) {
    ...
    for (let i = startIndex + 1; i < array.length; i++) {
        ...
        if (compareFunction(array[i], array[minIndex]) < 0) {
            ...
            indicating that we've found a new minimum element within the portion
